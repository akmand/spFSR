...
class SpFtSel_kernel:
    ...
    def set_inputs(self, x, y, wrapper):
        ...
        SpFtSel_log.logger.debug(f"feature names: {self._features_names}")
    ...
    def prepare_results_dict():
        ...
        SpFtSel_log.logger.debug('empty iter_results dictionary created')
        return iter_results

    def init_parameters(self):
        SpFtSel_log.logger.info(f'Algorithm run mode: {self._run_mode}')
        SpFtSel_log.logger.info(f"Number of features: {self._input_x.shape[1]}")
        SpFtSel_log.logger.info(f"Number of observations: {self._input_x.shape[0]}")
        # SpFtSel_log.logger.info('Setting initial parameters...')
        self._p = self._input_x.shape[1]
        SpFtSel_log.logger.debug(f'number of features : {self._p}')
        self._curr_imp = np.repeat(0.5, self._p)
        SpFtSel_log.logger.debug(f'curr imp : {self._curr_imp}')
        self._ghat = np.repeat(0.0, self._p)
        SpFtSel_log.logger.debug(f'ghat : {self._ghat}')
        self._curr_imp_prev = self._curr_imp
        SpFtSel_log.logger.debug(f'curr imp prev : {self._ghat}')
    ...
    def get_selected_features(self, imp):
        ...
        """
        SpFtSel_log.logger.debug("func get_selected_features:")
        ...
        SpFtSel_log.logger.debug(f"number of features to select: {num_features_to_select}")
    ...
    def gen_cv_task(self):
        if self._perf_eval_method is 'cv':
            if self._num_cv_reps_grad < 1:
                ...
                SpFtSel_log.logger.warning('cv repeats grad cannot be less than 1')

            if self._num_cv_reps_eval < 1:
                self._num_cv_reps_eval = 1
                SpFtSel_log.logger.warning('cv repeats eval cannot be less than 1')
    ...
    def eval_feature_set(self, cv_task, c_imp):
        ...
        SpFtSel_log.logger.debug(f'in eval_feature_set(), selected features: {selected_features}')
        ...
        if cv_task:
            ...
            SpFtSel_log.logger.debug('resubs error')
            ...
            best_value_std = 0.0
        SpFtSel_log.logger.debug(f"mean score: {best_value_mean}")
        SpFtSel_log.logger.debug(f"std score: {best_value_std}")
    ...
    def run_spFtSel(self):
        ...
        for iter_i in range(self._iter_max):
            ...
            SpFtSel_log.logger.debug(f'iteration gain smooth = {self._gain:1.3f}')
            SpFtSel_log.logger.debug(f'iteration gain raw = {self._raw_gain_seq[-1]:1.3f}')
            ...
            SpFtSel_log.logger.debug("same_feature_counter = {same_feature_counter}")
            ...
            if iter_i % self._print_freq == 0:
                SpFtSel_log.logger.info(f"iter: {iter_i}, value: {self._iter_results['values'][iter_i]}, "
